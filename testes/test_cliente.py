...
async def test_pesquisar_pelo_email():
    response = requests.get('http://localhost:8000/api/busca/cliente/juju@hotmail.com')
    status_code = response.status_code
    assert status_code == 200

@pytest.mark.asyncio
async def test_pesquisar_por_email_inexistente():
    response = requests.get('http://localhost:8000/api/busca/cliente/xpto@hotmail.com')
    status_code = response.status_code
...
async def test_remove_cliente_inexistente():
    response = requests.delete('http://localhost:8000/api/remove/cliente/xpto@hotmail.com')
    status_code = response.status_code
    assert status_code == 404
